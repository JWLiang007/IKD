...
def init_model_cam(args, cfg):
    ...
    for target_layer in args.target_layers:
        try:
            target_layers.append(eval(f'model.detector.{target_layer}')[-1])
...
def main():
    ...
    for image_path in images:
        ...
        image_with_bounding_boxes = det_cam_visualizer.show_cam(
            image, bboxes, labels, grayscale_cam, not args.no_norm_in_bbox,no_bbox_in_result=args.no_bbox_in_result)

        if args.out_path:
            if os.path.isdir(args.out_path):
                mmcv.mkdir_or_exist(args.out_path)
                out_file = os.path.join(args.out_path, os.path.basename(image_path))
            else:
