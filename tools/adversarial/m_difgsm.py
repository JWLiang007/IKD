...
class M_DIFGSM(Attack):
    ...
    def forward(self, data):
        ...
        Overridden.
        """
        batch_size = data['img'][0].data[0].shape[0]
        images = data['img'][0].data[0].clone().detach().to(self.device)
        ub,lb = torch.max(images.view(batch_size,3,-1),dim=-1).values,torch.min(images.view(batch_size,3,-1),dim=-1).values
        eps = self.eps * torch.max(ub - lb ,dim=-1).values
        alpha = self.alpha * torch.max(ub - lb,dim=-1).values
        ...
        if self.random_start:
            ...
            for i in range(batch_size):
                adv_images[i] = adv_images[i] + torch.empty_like(adv_images[i]).uniform_(-eps[i], eps[i])
                for  chn in range(adv_images[i].shape[0]):
                    adv_images[i,chn:chn+1,:,:] = torch.clamp(adv_images[i,chn:chn+1,:,:], min=lb[i,chn], max=ub[i,chn]).detach()
        new_data = {}
        ...
        for i in range(self.steps):
            ...
            adv_images = adv_images.detach() - alpha.view([-1] + [1] * (len(grad.shape) - 1)) * grad.sign()
            for j in range(batch_size):
                delta = torch.clamp(adv_images[j] - images[j], min=-eps[j], max=eps[j])
                for chn in range(adv_images[j].shape[0]):
                    adv_images[j,chn:chn+1,:,:] = torch.clamp(images[j,chn:chn+1,:,:] + delta[chn:chn+1,:,:], min=lb[j,chn], max=ub[j,chn]).detach()
